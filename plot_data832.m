...
% Use this code to eliminate the 24 cell well data from downstream analysis
for j = 1:length(green)
    listnum(j) = green(j).Nseed;
end

for j=1:length(listnum)
    if listnum(j)~=24
        keep(j) = true;
    else keep(j)=false;
    end
...
greennew = green(keep);
keepsum = logical([1 1 0 1]);
greensumnew = greensum(keepsum);
green = greennew;
greensum = greensumnew;
 %%       
figure (1)
subplot(2,3,1:3)
for j = 1:length(green)
