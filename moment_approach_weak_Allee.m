...
for j = 1:num_samps
    ...
    time(1)= 0;
for k = 2:num_iters
    birth_n = (b*N)*(1-((A+tau)./(N+tau))); % birth 
    if birth_n <0
        birth_n = 0;
    end
    death_n = (d*N)*(1-((A+tau)./(N+tau))); % death 
    if N==0
        N=0;
    else
        r = rand;
...
f = @(t,C) [((b-d)*C(1)*(1-((A+tau)./(C(1)+tau))));  % dN/dt
            ((1-((A+tau)/(C(1)+tau)))*(2*(C(2))*(b-d)) +  (C(1)*(b+d))*(1-((A+tau)/(C(1)+tau)))); %dN2/dt
            ((2*(C(2)-C(1))*(b-d)) +C(1)*(b+d))*(1-((A+tau)./(C(1)+tau)))]; %dvardt
...
figure;
plot(tsamp, mu_data(1:end), 'm*')
hold on
plot(tsamp, mu_C, 'k-', 'LineWidth',2)
xlabel('time (hours)')
ylabel('<n>')
title('Expected vs. simulated <n> weak Allee model')
...
xlabel('time (hours)')
ylabel('variance')
title('Expected vs. simulated var weak Allee model')
