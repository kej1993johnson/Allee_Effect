...
figure;
for i = 1:length(N1_init)
plot(ttot(:,i), N1tot(:,i), 'b-', 'LineWidth',2)
hold on
...
ylabel('Cell Number')
ylim([ 0 300])
title('dN/dt=g(N-A) solution')

figure;
for i = 1:length(N1_init)
plot(ttot(:,i), log(N1tot(:,i)./(N1_init(i))), 'b-', 'LineWidth',2)
hold on
end
ylim([ 0 5])
xlabel('time (hours)')
...
for i = 2%1:length(N1_init)
   ...
   plot(ttot(1:end-1,i), percapg(:,i), 'b.', 'LineWidth',2)
   hold off
   xlim([ 0 tsamp(end)])
   ylim([ 0 0.05])
   xlabel ('time')
   ylabel('per capita growth rate')
   title(['Per capita growth rate for dN/dt =g(N-A), N_{0}=',num2str(N1_init(i)),', A=', num2str(A)])
end
...
figure;
for i = 1:length(N4_init)
plot(ttot(:,i), N4tot(:,i), 'm-', 'LineWidth',2)
hold on
%text(ttot(end-20,i), N1tot(end-20,i),['Ni=', num2str(N1_init(i))]','HorizontalAlignment','left','VerticalAlignment','bottom','color','k' )
end
xlabel('time (hours)')
...
figure;
for i = 1:length(N4_init)
plot(ttot(:,i), log(N4tot(:,i)./(N4_init(i))), 'm-', 'LineWidth',2)
hold on
end
ylim([ 0 5])
xlabel('time (hours)')
...
for i = 2%1:length(N1_init)
    %subplot(1,length(N1_init),i)
   plot(ttot(1:end-1,i), percapg(:,i), 'm.', 'LineWidth',2)
   hold off
   xlim([ 0 tsamp(end)])
   ylim([ 0 0.05])
   xlabel ('time')
