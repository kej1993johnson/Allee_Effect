...
%% Plot fit and  b and d with CI for each initial cell number

mu_fitJst = mu_fxn(params_best_J,t, N0);
V_fitJst= V_fxn(params_best_J,t,N0, V0);
figure;
for j = 1:length(BTsum)-1
    params_best_J=BTsum(j).params_MC;
    subplot(1, length(BTsum), j)
    hold on
    plot(t, mu_fxn(params_best_J,t, BTsum(j).N0), 'LineWidth',2)
    plot(BTsum(j).timevec, BTsum(j).mu_t, 'LineWidth', 2, 'color', 'r')
    xlabel ('time')
    ylabel('mean')
    title(['N_{0}=',num2str(BTsum(j).N0)])%,', b=', num2str(round(params_best_J(1),2)),', d=', num2str(round(params_best_J(2),2))])
...
for j = 1:length(BTsum)-1
    ...
    subplot(1, length(BTsum), j)
    hold on
    plot(t, V_fxn(params_best_J,t, BTsum(j).N0, V0), 'LineWidth',2)
    plot(BTsum(j).timevec, BTsum(j).var_t, 'LineWidth', 2, 'color', 'g' )
    xlabel ('time')
    ylabel('variance')
    title(['N_{0}=',num2str(BTsum(j).N0)])%,', b=', num2str(round(params_best_J(1),2)),', d=', num2str(round(params_best_J(2),2))])
    %legend( 'model', 'measured')
...
figure;
for j = 1:length(BTsum)-1
    plot(BTsum(j).N0, BTsum(j).params_MC(1), 'bo', 'LineWidth',3)
    hold on
    plot(BTsum(j).N0, BTsum(j).params_MC(2), 'ro', 'LineWidth',3)
%     plot([BTsum(j).N0, BTsum(j).N0], BTsum(j).CrInt(:,1), 'b')
%     plot([BTsum(j).N0, BTsum(j).N0], BTsum(j).CrInt(:,2), 'r')
    xlabel ('Initial Cell Number (N_{0})')
    ylabel('Parameter Estimates of birth and death rates')
    title ('Estimate birth and death rates by N_{0}')
end

%% Plot some MCMC outputs for an individual fit
figure;
subplot(1,2,1)
plot(t, mudata, 'r*')
hold on
